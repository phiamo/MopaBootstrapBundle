...
class NavbarRenderer
{
     ...
     * @return string
     */
    public function renderTopNavbar($name, array $options = array())
    {
        ...
        ob_start();
        $template->displayBlock($block, array('topnavbar' => $navbar, 'options' => $options));
        $html = ob_get_clean();
     ...
     * @return string
     */
    public function renderMainNavbar($name, array $options = array())
    {
        $options = array_merge($this->getNavbarDefaultOptions(), $options);
        ...
        // we do not call renderBlock here to avoid too many nested level calls (XDebug limits the level to 100 by default)
        ob_start();
        $template->displayBlock($block, array('mainnavbar' => $navbar, 'options' => $options));
        $html = ob_get_clean();
